...
import { downloadSinglePdf, downloadBatchZip } from '../services/zipService';
...
export const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  const successCount = certs.filter(c => c.status === 'success').length;
  const failedCount = certs.filter(c => c.status === 'failed').length;

  let filteredCerts = certs.filter(c => {
    const matchesSearch = 
      c.primaryName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  filteredCerts.sort((a, b) => {
    if (sortBy === 'name') return a.primaryName.localeCompare(b.primaryName);
    if (sortBy === 'status') return a.status.localeCompare(b.status);
    return a.rowNumber - b.rowNumber;
  });

  const handleSingleDownload = async (cert: GeneratedCertificate) => {
    setDownloadingId(cert.id);
    try {
      let blob = cert.pdfBlob;
      if (!blob) {
        blob = await dbService.getPdfBlob(cert.id);
      }

      if (blob) {
        downloadSinglePdf(cert.id, cert.filename);
      } else {
        alert('PDF file binary missing in browser storage.');
      }
    } catch (err) {
      alert(`Failed to download certificate: ${err}`);
    } finally {
  ...
  const handleDownloadZip = async () => {
    setIsZipping(true);
    try {
      await downloadBatchZip(batch.id, batch.name, certs);
    } catch (err) {
      alert(`Failed to create ZIP package: ${err}`);
    } finally {
      setIsZipping(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <span>{isZipping ? 'Creating ZIP Package...' : 'Download All as ZIP'}</span>
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center space-x-3">
            <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-semibold">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${statusFilter === 'success' ? 'bg-white text-emerald-700 shadow-2xs' : 'text-slate-600'}`}
              >
                ...
                filteredCerts.map((cert) => (
                  <tr key={cert.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-semibold text-slate-900">
                      {cert.primaryName || `Record ${cert.rowNumber}`}
                    </td>
